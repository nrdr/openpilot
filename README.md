...
Settings appear only when they apply to the detected car and hardware. A grayed-out control may be unavailable for the detected car, locked while steering is engaged, waiting for its master switch, or owned by **Handcrafted Lateral Tuning**. Steer-ratio mode and manual ratio controls remain independent from the Handcrafted profile. Start from defaults, change one thing at a time, and never use a slider to hide a firmware mismatch.

<details>
<summary><strong>Handcrafted Lateral Tuning and vehicle learning</strong></summary>

- **Handcrafted Lateral Tuning** loads the versioned, Clarity-derived road-tested controller profile for the supported Accord, Civic, Clarity, CR-V, and Insight fingerprints. It restores the controller values it owns, but it does not choose or overwrite your steer-ratio mode.
