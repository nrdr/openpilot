...
The Handcrafted profile does not lock these controls; after a one-shot apply, manual changes persist. Do not max either control: higher thresholds make NRDR slower to yield to real driver input. This is only a false-detection workaround, not a cure. If stutter is strong, repeatable, or affects control, stop testing that firmware, return to a known-good configuration, and report the exact rack/firmware plus logs in Discord.

Many Hondas also have a non-linear variable steering ratio inside the rack. A single static ratio cannot describe the whole steering range, so the car may track too far inside or outside a curve when the software and rack geometry disagree. **Steer Ratio Tuning** now makes the source explicit: Manual, comma's scalar learner, NRDR's raw Clarity road measurements, or an exact EPS-firmware map. Unsupported Raw or Firmware choices visibly fall back to the car's stock ratio instead of borrowing another car's data.
...
Settings appear only when they apply to the detected car and hardware. A grayed-out control may be unavailable for the detected car, locked while steering is engaged, or waiting for its master switch. Start from defaults, change one thing at a time, and never use a slider to hide a firmware mismatch.

<details>
<summary><strong>Handcrafted Lateral Tuning and vehicle learning</strong></summary>

- **Apply Handcrafted Lateral Profile** is a durable one-shot command. Press APPLY offroad; the device writes the versioned profile in a fail-safe order, verifies every value, records the result, and turns the request off. It never keeps restoring values, so every manual edit afterward persists. Clarity v17 is offered only for the exact modified-EPS Clarity with its Firmware steer-ratio map and Torque/PIF controller support; the other supported Accord, Civic, CR-V, and Insight fingerprints retain their safe steer-ratio-independent v15 profiles.
...
- **Interpolated Torque/PIF Blend** is available only on a supported modified-EPS Honda using NRDR's PID or Clarity hybrid. OFF leaves the complete P/I/F answer untouched. ON calculates the complete steering-angle-based P/I/F answer and a separate classic Torque answer, then makes one weighted final command. The Torque side follows f13's generic yaw-feedback branch, which was not historically road-proven on Honda. It uses the selected steering geometry at or below `2 m/s`, blends from angle to calibrated yaw response between `2–5 m/s`, and uses calibrated yaw response at or above `5 m/s`. Whenever yaw is required above `2 m/s` but is missing, stale, invalid, or non-finite, the final command temporarily returns to exact P/I/F and the entire Torque controller holds its state until valid yaw returns; it does not substitute angle feedback. **Torque Share** sets the weight: `30%` Torque means `30%` classic Torque and `70%` P/I/F, not two commands stacked together. **Spoofed Lateral Acceleration Factor** controls only the Torque side's non-friction error and feedforward (`5.0 m/s²` default). Three direct friction controls—**Low-Speed Torque Friction**, **Standard-Speed Torque Friction**, and **Highway Torque Friction**—are independent of that factor and default to `0.12`, `0.10`, and `0.06`. Low is exact through `24 mph`, blends to Standard from `24–26 mph`, Standard is exact from `26–49 mph`, blends to Highway from `49–51 mph`, and Highway is exact from `51 mph` up. Equal values preserve the prior single-friction behavior exactly. Friction ramps with lateral-acceleration error and stays at the selected speed-banded amount once that error reaches `0.3 m/s²`; it does not decay with time. All six settings latch for one engagement and the learner pauses while the blend is active. On the Clarity, the blend takes priority: NNLC stays configured but is bypassed and fully reset, then becomes available again after the blend is turned off.
- **Steer Ratio Tuning** chooses exactly one source. **Use Comma Steer Ratio Learner** starts from the car's stock number, learns one new number, and keeps the last good result through brief dropouts. **Use nrdr Steer Ratio Learner** uses audited Clarity road data through a left-and-right point at `435.7°`, near full lock. NRDR fills the spaces between recorded points, converts the logged turn geometry into the effective ratio the vehicle model needs, and clamps only past that supported endpoint. **Use Firmware Steer Ratio** uses a recognized steering-rack map anchored to the car's stock number. Turning all three switches OFF selects Manual on supported Hondas; other cars keep their stock ratio.
- **Manual Override On-Center Ratio / Final Ratio** are the two Manual endpoints. The first controls steering near straight ahead; the second controls steering farther into a turn. NRDR blends between them using the detected rack's steering range. The defaults are `15.38` and `10.93`.
...
- **NNLC** is the optional Clarity neural lateral controller. Activation speed chooses the PID-to-NNLC handoff; KP, KI, and KF scale its three terms. Lane changes remain on PID, and Firmware Steer Ratio mode disables NNLC when an exact firmware map is active.
