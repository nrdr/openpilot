...
class CarInterface(CarInterfaceBase):
  ...
  def torque_from_lateral_accel_modded(self, latcontrol_inputs: LatControlInputs, torque_params: car.CarParams.LateralTorqueTuning, lateral_accel_error: float, lateral_accel_deadzone: float, friction_compensation: bool, gravity_adjusted: bool) -> float:
    ...
    mod_factor = 1.8333333 # Lateral Accel
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    if candidate == CAR.HONDA_CIVIC:
      if eps_modified:
        ...
        ret.lateralTuning.pid.kf = 0.0000267 # conservative feed-forward
        ...
        ret.lateralTuning.pid.kpBP, ret.lateralTuning.pid.kpV = [[0, 22, 27], [0.0223, 0.0223, 0.0223]]
        ret.lateralTuning.pid.kiBP, ret.lateralTuning.pid.kiV = [[0, 22, 27], [0.0092, 0.0092, 0.0092]]
      else:
        ret.lateralTuning.pid.kf = 0.00006  # conservative feed-forward
        ret.lateralParams.torqueBP = [0x0, 0x917, 0xDC5, 0x1017, 0x119F, 0x140B, 0x1680, 0x4EC0, 0x5A00]
        ret.lateralParams.torqueV = [0x0, 0x200, 0x300, 0x478, 0x5EC, 0x800, 0xA00, 0xE00, 0xF00]
