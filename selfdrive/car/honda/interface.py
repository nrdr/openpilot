  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    if candidate == CAR.HONDA_CIVIC:
      if eps_modified:
        ...
        ret.lateralParams.torqueBP = [0, 2327, 3525, 4119, 4511, 5131, 5760, 22464, 25344]
        ret.lateralParams.torqueV = [0, 512, 768, 1144, 1516, 2048, 2560, 3584, 3840]
        ret.lateralTuning.pid.kpBP, ret.lateralTuning.pid.kpV = [[0, 4.4704, 13.4112, 20.1168], [0.1, 0.147, 0.147, 0.147]]   # 0 / 10 / 30 / 45 MPH
        ret.lateralTuning.pid.kiBP, ret.lateralTuning.pid.kiV = [[0, 4.4704, 13.4112, 20.1168], [0.049, 0.049, 0.049, 0.049]]  # 0 / 10 / 30 / 45 MPH
      else:
        ret.lateralTuning.pid.kf = 0.00006  # conservative feed-forward
