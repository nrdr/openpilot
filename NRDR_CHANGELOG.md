...
- **Live Learning Gas default is now hardware-aware** — fresh installs default `HondaLiveLearningGas` OFF when a gas pedal interceptor is detected and ON otherwise. The persistent value is only initialized once, so later user changes are preserved.
