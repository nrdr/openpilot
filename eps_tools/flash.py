...
def stop_openpilot():
    ...
    subprocess.run(["pkill", "-f", "openpilot"], check=False)
    time.sleep(2)
    print("  done.\n")
...
def main():
    ...
    if res.stderr:
        print(res.stderr)
    if DRY_RUN_OK_MARKER not in (res.stdout + res.stderr):
        sys.exit("\nDry run did NOT reach the safe abort point — NOT flashing. Review the output above.")
    print("Dry run OK — reached the safe abort point (the traceback above is expected for a dry run).")
    ...
    if input("Type 'FLASH' to commit the real flash (--danger): ").strip() != "FLASH":
        sys.exit("Aborted — no flash performed.")
