...
# These constants approximate the integer MPH ladder Honda clusters display in imperial mode.
HONDA_MPH_PER_KPH = 0.6233
HONDA_MPH_OFFSET = 0.0995


def _honda_ms_to_mph_int(v_ms: float) -> int:
  """Convert m/s to the integer MPH value a Honda cluster would display in imperial mode."""
...
def compare_cluster_target(v_cruise_cluster: float,
  ...
  For most cars:
    - Converts m/s to rounded KPH/MPH with standard unit conversions.

  For Honda in imperial mode:
    - Uses the Honda-specific MPH ladder mapping (kph*0.6233 + 0.0995) before comparing.
  """
