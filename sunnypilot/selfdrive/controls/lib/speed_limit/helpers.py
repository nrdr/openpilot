...
def _honda_ms_to_mph_int(v_ms: float) -> int:
  # Convert m/s -> kph, then use Honda's mph ladder mapping
...
def compare_cluster_target(v_cruise_cluster: float,
  ...
  For most cars: uses round(ms->kph/mph).
  For Honda in imperial mode: uses Honda's cluster MPH ladder mapping (kph*0.6233 + 0.0995).
  """
