...
# The cluster/UI set speed remains unchanged, while the longitudinal control
# target may be adjusted to account for vehicle-specific speed reporting and
# control behavior.
# -----------------------------------------------------------------------------
...
class SpeedLimitAssist:
  ...
  def __init__(self, CP: car.CarParams, CP_SP: custom.CarParamsSP):
    ...
    # Vehicle-specific bias applied to control targets while preserving cluster/UI semantics.
    self.speed_bias = SPEED_BIAS_CLARITY if CP.carFingerprint == CAR.HONDA_CLARITY else SPEED_BIAS_DEFAULT

    # Enable Honda's imperial MPH ladder mapping only for Clarity to avoid altering other Honda platforms.
    self.honda_imperial = (CP.carFingerprint == CAR.HONDA_CLARITY)
  ...
  def get_v_target_from_control(self) -> float:
    if self._has_speed_limit:
      # When SLA is in control, output the biased control target while all confirmation and UI logic
      # remains anchored to the unbiased target.
  ...
  def update_calculations(self, v_cruise_cluster: float) -> None:
    ...
    # Apply a control-only bias while keeping cluster/UI comparisons tied to the unbiased target.
    self._speed_limit_final_last_ctrl = self._speed_limit_final_last * self.speed_bias

    # Update current velocity offset (error) using the control target.
    self.v_offset = self._speed_limit_final_last_ctrl - self.v_ego

    # Conversions used for confirm logic/UI comparisons remain unbiased.
    ...
    pcm_long_required_max = (
      cst_low if self._has_speed_limit and self.speed_limit_final_last_conv < CONFIRM_SPEED_THRESHOLD[self.is_metric]
      else cst_high
    )
  ...
  def _update_confirmed_state(self) -> None:
    if self._has_speed_limit:
  ...
  def _update_non_pcm_long_confirmed_state(self) -> bool:
    ...
    req_plus, req_minus = compare_cluster_target(
      ...
      self.is_metric,
      honda_imperial=(not self.is_metric) and self.honda_imperial,
    )
