...
Lateral Tuning. Ford only - these have no effect on other makes. Everything here is
gated behind the master toggle, which defaults OFF (stock curvature-only steering).
"""
...
class FordLateralTuningLayout(Widget):
  ...
  def _initialize_items(self):
    self._ford_oem_lateral = toggle_item_sp(
      ...
      description=lambda: tr("Ford only. Replaces stock curvature-only steering with Ford's full four-signal command (adds curvature-rate plus a lane-centering path-angle), the way the factory system actually drives. Targets the two stock complaints: the wheel fighting you after you grab it, and running out of steering authority in vigorous turns. OFF = stock curvature-only path, completely unchanged. BETA - first test on an empty road with your hands ready. No effect on non-Ford cars."),
    )

    self._ford_human_turn = toggle_item_sp(
      param="NrdrFordHumanTurn",
      title=lambda: tr("Ford: Human Turn Detection (Default: ON)"),
      description=lambda: tr("When you hold the wheel into a turn past 45 degrees, openpilot stops commanding steering so it isn't fighting you, then ramps smoothly back in when you let go instead of snapping. This is the fix for 'the wheel tries to throw me off the road' after you touch it. Only active when OEM-Style Lateral is on."),
    )

    self._ford_lane_positioning = toggle_item_sp(
      param="NrdrFordLanePositioning",
      title=lambda: tr("Ford: Lane Positioning (Default: ON)"),
      description=lambda: tr("Drives the path-angle signal with a lane-centering PID for extra steering authority and tighter in-lane position - the main lever against 'runs out of torque'. Engages at highway speed and steps aside during auto lane changes. Only active when OEM-Style Lateral is on."),
    )
    ...
    self._ford_lane_pos_gain = option_item_sp(
      ...
      max_value=300,
      value_change_step=5,
      description=lambda: tr("How hard the lane-centering path-angle pulls toward lane center. 100% = baseline. Raise for stronger centering, lower if it feels busy. Only used when Lane Positioning is on."),
  ...
  def _update_state(self):
    super()._update_state()
    # Reveal the sub-options only once the master toggle is on.
