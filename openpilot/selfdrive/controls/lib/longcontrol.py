...
def long_control_state_trans(CP_SP, active, long_control_state,
                             ...
                             CP=None, v_ego=0.0, v_ego_starting=None):
  # Gas Interceptor
  ...
  starting_condition = (not should_stop and
                        ...
                        not brake_pressed)
  started_condition = CP is None or v_ego > (CP.vEgoStarting if v_ego_starting is None else v_ego_starting)
  ...
  if not active:
    ...
    if long_control_state == LongCtrlState.off:
      ...
      elif CP is not None and CP.startingState:
        long_control_state = LongCtrlState.starting
      else:
        long_control_state = LongCtrlState.pid

    elif long_control_state == LongCtrlState.stopping:
      if starting_condition and CP is not None and CP.startingState:
        long_control_state = LongCtrlState.starting
...
class LongControl:
  def __init__(self, CP, CP_SP):
    ...
    self.scale_excludes_kf = True  # when True, PID scale multiplies P+I only, not feedforward (StaticFeedforwardLong, default ON)
  ...
  def _read_live_params(self):
    ...
    self.scale_excludes_kf = self.params.get_bool("StaticFeedforwardLong")
  ...
  def update(self, active, CS, a_target, should_stop, accel_limits, pitch=None, drel=None):
    ...
    self.long_control_state = long_control_state_trans(self.CP_SP, active, self.long_control_state,
                                                       ...
                                                       CS.cruiseState.standstill, CP=self.CP, v_ego=CS.vEgo,
                                                       v_ego_starting=self.v_ego_starting)
