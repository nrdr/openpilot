...
CANONICAL_DEVICE = REPOSITORY_ROOT / "openpilot" / "sunnypilot" / "nrdr" / "tools" / "device"
...
def test_compatibility_paths_are_thin_argument_preserving_launchers() -> None:
  for helper in HELPERS:
    ...
    assert policy_lines == [
      ...
      f'exec "$REPO_ROOT/openpilot/sunnypilot/nrdr/tools/device/{helper}" "$@"',
    ]
...
def test_boot_calls_the_canonical_setup_owner() -> None:
  ...
  canonical_path = "openpilot/sunnypilot/nrdr/tools/device/setup_dev_helpers.sh"
...
def test_compatibility_launchers_preserve_arguments_and_exit_status(tmp_path: Path, helper: str) -> None:
  ...
  implementation = tmp_path / "openpilot" / "sunnypilot" / "nrdr" / "tools" / "device" / helper
...
def test_device_helpers_ship_in_release() -> None:
  ...
  expected = {
    ...
    *(f"openpilot/sunnypilot/nrdr/tools/device/{helper}" for helper in HELPERS),
    "openpilot/sunnypilot/nrdr/tools/device/README.md",
  }
