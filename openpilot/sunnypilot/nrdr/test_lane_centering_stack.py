...
SUNNYLINK_STEERING = SUNNYLINK_ROOT / "settings_ui_src/pages/steering.yaml"
...
class TestLaneCenteringStack(unittest.TestCase):
  ...
  def test_sunnylink_stack_is_nested_under_nrdr_lateral_tuning(self) -> None:
    ...
    source_steering = yaml.safe_load(SUNNYLINK_STEERING.read_text(encoding="utf-8"))
    ...
    nrdr_section = next(section for section in source_steering["sections"] if section["id"] == "nrdr")
    ...
    self.assertEqual(sibling_ids, [
      "nrdr_vehicle_model_learning",
      "nrdr_pidf_ground",
      "nrdr_lane_centering_stack",
    ...
    self.assertEqual([panel["id"] for panel in compiled_nrdr["sub_panels"]], sibling_ids)
    ...
    for path in (SUNNYLINK_ROOT / "settings_ui_src").rglob("*.yaml"):
  ...
  def test_migration_is_restrictive_and_does_not_enable_lane_centering(self) -> None:
    ...
    self.assertIn('{"LaneCenteringMinSpeed", {PERSISTENT | BACKUP, INT, "50"}}', source)
    self.assertIn("Existing enabled installs adopt the new 50 mph minimum", SUNNYLINK_STEERING.read_text(encoding="utf-8"))
