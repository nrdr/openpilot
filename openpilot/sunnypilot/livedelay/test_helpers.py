...
def test_lagd_toggle_caches_the_effective_delay(monkeypatch):
  ...
  msg = SimpleNamespace(liveDelay=SimpleNamespace(lateralDelay=0.37))

  toggle.update(msg)
  assert toggle.lag == pytest.approx(1.10)
  assert msg.liveDelay.lateralDelay == pytest.approx(1.10)
  assert params.writes["LagdValueCache"] == pytest.approx(1.10)

  params.values["LagdToggle"] = True
  msg.liveDelay.lateralDelay = 0.37
  toggle.update(msg)
  assert toggle.lag == 0.37
  assert msg.liveDelay.lateralDelay == 0.37
...
def test_lagd_toggle_publishes_effective_delay_without_excess_cache_writes(monkeypatch):
  ...
  monkeypatch.setattr(lagd_toggle, "Params", lambda: params)
  toggle = lagd_toggle.LagdToggle(SimpleNamespace(steerActuatorDelay=0.10))
  msg = SimpleNamespace(liveDelay=SimpleNamespace(lateralDelay=0.37, lateralDelayEstimate=0.41))

  toggle.update(msg, cache=False)
  assert msg.liveDelay.lateralDelay == pytest.approx(0.60)
  assert msg.liveDelay.lateralDelayEstimate == pytest.approx(0.41)
...
def test_lagd_selects_effective_delay_before_serializing_and_publishing():
  ...
  publication = source.index("pm.send('liveDelay', lag_msg_dat)", serialization)
...
def test_controlsd_refreshes_each_published_live_delay():
  openpilot_root = Path(__file__).resolve().parents[2]
  controlsd_source = (openpilot_root / "selfdrive/controls/controlsd.py").read_text(encoding="utf-8")
  assert 'if self.sm.updated["liveDelay"]:' in controlsd_source
  assert 'get_lat_delay(self.params, self.sm["liveDelay"].lateralDelay, self.CP.steerActuatorDelay)' in controlsd_source


def test_modeld_refreshes_each_published_live_delay():
  openpilot_root = Path(__file__).resolve().parents[2]
  for relative_path in ("selfdrive/modeld/modeld.py", "sunnypilot/modeld_v2/modeld.py"):
    ...
    assert 'if sm.updated["liveDelay"]:' in source
