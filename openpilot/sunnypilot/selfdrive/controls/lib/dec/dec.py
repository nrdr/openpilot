...
class ModeTransitionManager:
  ...
  def __init__(self, *, nrdr_enabled: bool = False):
    self.nrdr_enabled = nrdr_enabled
  ...
  def request_mode(self, mode: ModeType, confidence: float = 1.0, emergency: bool = False):
    ...
    enforce_dwell = (enforce_mode_dwell(self, mode) if self.nrdr_enabled else
                     self.mode_duration < self.min_mode_duration and not self.emergency_override)
    if enforce_dwell:
      return
...
class DynamicExperimentalController:
  def __init__(self, CP: structs.CarParams, mpc, params=None):
    self._CP = CP
    self._nrdr_enabled = nrdr_longitudinal_enabled(CP)
    self._mpc = mpc
    ...
    self._mode_manager = ModeTransitionManager(nrdr_enabled=self._nrdr_enabled)
  ...
  def _calculate_slow_down(self, md):
    ...
    if not (position_valid and orientation_valid):
      ...
      threshold = (slow_down_threshold(WMACConstants.SLOW_DOWN_PROB, self._has_slow_down, 1.0, 0.625)
                   if self._nrdr_enabled else WMACConstants.SLOW_DOWN_PROB)
    ...
    threshold = (slow_down_threshold(WMACConstants.SLOW_DOWN_PROB, self._has_slow_down, 0.8, 0.5)
                 if self._nrdr_enabled else WMACConstants.SLOW_DOWN_PROB * 0.8)
    self._has_slow_down = urgency_filtered > threshold
    self._urgency = urgency_filtered
