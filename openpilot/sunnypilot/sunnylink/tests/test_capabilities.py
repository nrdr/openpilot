...
class TestOpaquePerBrandFlags(OpenpilotTestCase):
  ...
  def test_exact_honda_bundle_enables_manual_and_clarity_raw(self, params):
    ...
    assert model_caps["nrdr_honda_tuning_available"] is False
  ...
  def test_legacy_handcrafted_bundle_remains_available_without_clarity_extensions(self, params):
    ...
    put_car_params(params, "HONDA_ACCORD", "honda")
    assert generate_capabilities(params)["has_handcrafted_lateral_profile"] is True
    assert generate_capabilities(params)["nrdr_honda_tuning_available"] is True
