...
class TestModelBundleSlotMigration(OpenpilotTestCase):
  ...
  def test_legacy_usbgpu_slot_takes_priority(self):
    params = Params()
    params.put("ModelManager_ActiveBundle", {"ref": "small"}, block=True)
    params.put("ModelManager_ActiveBundleUSBGPU", {"ref": "legacy-big"}, block=True)
    _migrate_model_bundle_slots(params)
    assert params.get("ModelManager_ActiveBundleChestnut") == {"ref": "legacy-big"}
