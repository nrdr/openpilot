...
The three values are published as one background snapshot and captured once per control frame. Measured-curvature and PID desired-angle paths use the same complete selection, including during an active engagement. Values may be saved onroad and normally reach the controller within 0.5 seconds; no disengagement or restart is needed. Torque-controlled cars retire a tuning-induced command step over one second. This transition does not suppress driver overrides or replace existing curvature, jerk, or torque limits. Have a passenger make adjustments while driving. Background polling is not a hard real-time delivery guarantee, and separately saved edits are not a storage transaction.
