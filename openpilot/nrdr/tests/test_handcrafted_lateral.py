...
@pytest.mark.parametrize("fingerprint", EXPECTED_FINGERPRINTS)
def test_profile_does_not_own_or_advertise_steer_ratio_geometry(fingerprint):
  profile = get_handcrafted_lateral_profile(fingerprint)
  values = dict(profile.values)

  assert not set(values) & RETIRED_STEER_RATIO_KEYS
  assert "NrdrSteerRatioMode" not in values
  assert "NrdrSteerRatioManualCenter" not in values
  assert "NrdrSteerRatioManualFinal" not in values
...
def test_profile_preserves_the_current_road_tested_choices():
  ...
  for fingerprint in HONDA_TORQUE_MOD_HANDCRAFTED_FINGERPRINTS:
    ...
    assert not RETIRED_STEER_RATIO_KEYS & profile_values.keys()
