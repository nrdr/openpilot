...
def test_vehicle_model_policy_respects_each_learning_gate():
  ...
  assert vehicle_model_params(controls, live_params) == (1.0, 16.5, 0.0)

  controls.learn_stiffness = True
  controls.learn_steer_ratio = False
  controls.learn_angle_offset = True
  assert vehicle_model_params(controls, live_params) == (0.8, 15.0, 1.2)


def test_vehicle_model_policy_preserves_positive_floor():
  controls = SimpleNamespace(
  ...
  assert vehicle_model_params(controls, live_params) == (0.1, 0.1, 0.0)


def test_stopping_inputs_and_hud_lead_translate_framework_messages():
  pose = SimpleNamespace(orientation=SimpleNamespace(xyz=(0.0, 0.04, 0.0)))
  plan = SimpleNamespace(hasLead=True, leadTrajectoryX0=(21.5,))
  assert stopping_inputs(pose, plan) == (0.04, 21.5)
  assert stopping_inputs(None, SimpleNamespace(hasLead=False, leadTrajectoryX0=())) == (None, None)

  hud = SimpleNamespace(leadDistance=-1.0, leadVLead=-1.0)
  apply_hud_lead(hud, SimpleNamespace(present=True, dRel=21.5, vLead=17.0))
  assert (hud.leadDistance, hud.leadVLead) == (21.5, 17.0)
  apply_hud_lead(hud, SimpleNamespace(present=False, dRel=99.0, vLead=99.0))
  assert (hud.leadDistance, hud.leadVLead) == (0.0, 0.0)


def test_driver_monitoring_policy_retains_disabled_timeout_values():
  settings = SimpleNamespace()
  apply_driver_monitoring_policy(settings)

  assert settings._WHEELTOUCH_POLICY_ALERT_1_TIMEOUT == 86400.0 - 15.0
  assert settings._WHEELTOUCH_POLICY_ALERT_2_TIMEOUT == 86400.0 - 6.0
  assert settings._WHEELTOUCH_POLICY_ALERT_3_TIMEOUT == 86400.0
  assert settings._VISION_POLICY_ALERT_1_TIMEOUT == 86400.0 - 8.0
  assert settings._VISION_POLICY_ALERT_2_TIMEOUT == 86400.0 - 6.0
  assert settings._VISION_POLICY_ALERT_3_TIMEOUT == 86400.0
  assert settings._NO_RESPONSE_TIMEOUT == 86400.0
  assert settings._MAX_ALERT_3 == 86400
  assert settings._MAX_NO_RESPONSE == 86400


def test_auto_lkas_rearms_only_after_main_becomes_unavailable():
  auto_lkas = AutoLkas()
  added = []
  ...
  auto_lkas.request(available, True, events, event_name)
  assert added == ["lkasEnable", "lkasEnable"]
