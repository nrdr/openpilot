...
CLEAN_OVERLAY_DIR="$SOURCE_DIR/openpilot/nrdr/tools/release/clean_overlay"
BUILD_READY=0
BUILD_DIR_OWNED=0
...
validate_source_tree() {
  local item validation_error="" has_version=0 has_helper=0 has_launcher=0 has_builder=0 has_backend_env=0
  validate_layout
  ...
  for item in "${RELEASE_FILES[@]}"; do
    ...
    [ "$item" = "release/build_prebuilt.sh" ] && has_launcher=1
    [ "$item" = "openpilot/nrdr/tools/release/build_prebuilt.sh" ] && has_builder=1
    [ "$item" = "openpilot/nrdr/config/backend_env.sh" ] && has_backend_env=1
  done
  set -x
  ...
  [ "$has_launcher" = "1" ] || die "release manifest omits the stable prebuilt launcher"
  [ "$has_builder" = "1" ] || die "release manifest omits the NRDR prebuilt implementation"
  [ "$has_backend_env" = "1" ] || die "release manifest omits the NRDR backend policy"
...
apply_clean_overlay() {
  ...
  local backend_target="$BUILD_DIR/openpilot/nrdr/config/backend_env.sh"
  ...
  [ -f "$backend_target" ] || die "clean backend policy target is missing: $backend_target"
  ...
  ! grep -Fq 'konik.ai' "$backend_target" || die "clean backend policy still selects Konik"
  grep -Fq 'openpilot/nrdr/config/backend_env.sh' "$BUILD_DIR/launch_env.sh"
