...
Usage:
    py -3.13 -m openpilot.nrdr.tools.radar_re.cli --captures <path1.csv> [<path2.csv> ...] \\
...
# Ensure the worktree root is on sys.path when this file is invoked directly.
_wt_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
if _wt_root not in sys.path:
    sys.path.insert(0, _wt_root)

from openpilot.nrdr.tools.radar_re import BOSCH_RADAR_ALL_IDS
from openpilot.nrdr.tools.radar_re.ingest import load, rlog_available
...
from openpilot.nrdr.tools.radar_re.sweep import score_windows
from openpilot.nrdr.tools.radar_re.enums import detect_all_enums
from openpilot.nrdr.tools.radar_re.gates import apply_batch
from openpilot.nrdr.tools.radar_re.report import write_report
