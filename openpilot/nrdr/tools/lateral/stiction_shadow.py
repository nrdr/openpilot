...
from openpilot.nrdr.tools.steer_ratio.correction import expand_sources, iter_local_events, route_segment
...
def iter_shadow_frames(source: str) -> Iterable[ShadowFrame]:
  ...
  for event in iter_local_events(source):
    ...
    if which == "initData":
      ...
      angle_offset_deg = float(event.vehicleParameters.angleOffsetDeg) if learn_angle_offset else 0.0
    elif which == "modelV2":
