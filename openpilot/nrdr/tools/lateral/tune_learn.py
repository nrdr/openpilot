...
Usage:
  python3 -m openpilot.nrdr.tools.lateral.tune_learn view
  python3 -m openpilot.nrdr.tools.lateral.tune_learn view /data/media/0/realdata/*/rlog.zst
  python3 -m openpilot.nrdr.tools.lateral.tune_learn seed /data/media/0/realdata/*/rlog.zst
"""
import sys
import numpy as np
...
def _print_residual(paths):
  from openpilot.nrdr.tools.lateral.tune_grid import collect, build_grid, max_used_angle_bin, print_grid
...
def cmd_seed(paths, file=None, gain=SEED_GAIN, steer_max=STEER_MAX):
  if not paths:
    ...
    return
  from openpilot.nrdr.tools.lateral.tune_grid import collect, build_grid, cell_stats, MIN_CELL
  errs = build_grid(collect(paths))
